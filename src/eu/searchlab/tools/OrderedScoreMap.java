...
public class OrderedScoreMap<E> extends AbstractScoreMap<E> implements ScoreMap<E> {
    ...
    public static void main(final String[] args) {
        final OrderedScoreMap<StringBuilder> w = new OrderedScoreMap<StringBuilder>(StringBuilderComparator.CASE_INSENSITIVE_ORDER);
        final Random r = new Random();
        for (int i = 0; i < 10000; i++) {
            w.inc(new StringBuilder("a" + ((char) (('a') + r.nextInt(26)))));
        }
        for (final StringBuilder s: w) System.out.println(s + ":" + w.get(s));
        System.out.println("--");
